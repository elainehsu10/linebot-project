...
def _verify_line_id_token(id_token: str) -> str | None:
    ...
    if not id_token:
        current_app.logger.error("ID Token 為空")
        return None
    
    try:
        import os
        env_value = os.environ.get('LIFF_CHANNEL_ID')
        config_value = current_app.config.get('LIFF_CHANNEL_ID')
        
        current_app.logger.info(f"🔍 Token 長度: {len(id_token)}")
        current_app.logger.info(f"🔍 Token 前20字符: {id_token[:20]}...")
        ...
        if not client_id:
            current_app.logger.error("LIFF_CHANNEL_ID 未設定")
            return None
        ...
        if response.status_code != 200:
            current_app.logger.warning(f"🔍 LINE API 回應內容: {response.text}")
            return None
        
        result = response.json()
        user_id = result.get('sub')
        current_app.logger.info(f"✅ ID Token 驗證成功 - user_id: {user_id}")
        return user_id
        
    except requests.exceptions.Timeout:
        current_app.logger.error("LINE API 請求超時")
        return None
        ...
        current_app.logger.error(f"ID Token 驗證過程中發生未預期錯誤: {e}")
        return None
...
def create_reminders_from_prescription_api():
    try:
        ...
        if not auth_header.startswith('Bearer '):
            current_app.logger.error("缺少或無效的 Authorization header")
            return jsonify({"success": False, "message": "缺少認證資訊"}), 401
        
        id_token = auth_header.split(' ')[1]
        user_id = _verify_line_id_token(id_token)
        if not user_id:
            current_app.logger.error("ID Token 驗證失敗")
            return jsonify({"success": False, "message": "無效的 Token"}), 401
        
        # 獲取提醒資料
        request_data = request.get_json()
        if not request_data:
            current_app.logger.error("請求中沒有 JSON 資料")
            return jsonify({"success": False, "message": "請求格式錯誤"}), 400
        
        reminders = request_data.get('reminders', [])
        if not reminders:
            current_app.logger.error("沒有收到提醒資料")
            return jsonify({"success": False, "message": "沒有收到提醒資料"}), 400
        
        current_app.logger.info(f"用戶 {user_id} 嘗試建立 {len(reminders)} 個提醒")
        
        # 驗證提醒資料格式
        for i, reminder in enumerate(reminders):
            required_fields = ['recorder_id', 'member', 'drug_name']
            for field in required_fields:
                if not reminder.get(field):
                    current_app.logger.error(f"提醒 {i+1} 缺少必要欄位: {field}")
                    return jsonify({"success": False, "message": f"提醒資料格式錯誤：缺少 {field}"}), 400
        
        # 建立提醒
        success = reminder_service.ReminderService.create_reminders_batch(reminders, user_id)
        
        if success:
            current_app.logger.info(f"用戶 {user_id} 成功建立提醒")
            ...
            try:
                ...
                # 從第一個提醒中獲取成員資訊
                if reminders:
                    member_name = reminders[0].get('member')
                    if member_name:
                        # 獲取該成員的所有提醒
                        member_reminders = reminder_service.ReminderService.get_reminders_for_member(user_id, member_name)
                        
                        # 生成提醒列表 Flex Message
                        liff_id = current_app.config['LIFF_ID_MANUAL_REMINDER']
                        
                        # 構造成員資訊
                        member_info = {'member': member_name, 'id': None}
                        flex_message = flex_reminder.create_reminder_list_carousel(member_info, member_reminders, liff_id)
                        
                        # 發送 Push Message
                        line_bot_api.push_message(user_id, flex_message)
                        current_app.logger.info(f"已向用戶 {user_id} 發送 {member_name} 的處方提醒列表")
            ...
            current_app.logger.error(f"用戶 {user_id} 建立提醒失敗")
            return jsonify({"success": False, "message": "儲存提醒失敗"}), 500
            
    except Exception as e:
        current_app.logger.error(f"處理批量建立提醒請求時發生錯誤: {e}")
        import traceback
