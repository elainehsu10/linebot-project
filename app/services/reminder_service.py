...
class ReminderService:
    ...
    def create_reminders_batch(reminders_data: list, user_id: str):
        """批量建立來自藥單的提醒"""
        try:
            print(f"[DEBUG] 開始批量建立提醒，用戶: {user_id}, 提醒數量: {len(reminders_data)}")
            
            # 驗證權限
            for i, r in enumerate(reminders_data):
                if r.get('recorder_id') != user_id:
                    print(f"[ERROR] 提醒 {i} 權限檢查失敗: {r.get('recorder_id')} != {user_id}")
                    raise PermissionError("沒有權限建立不屬於自己的提醒")
                print(f"[DEBUG] 提醒 {i}: {r.get('drug_name', 'Unknown')} for {r.get('member', 'Unknown')}")
            
            result = DB.create_reminders_batch(reminders_data)
            print(f"[DEBUG] 批量建立提醒結果: {result}")
            return result
            
        except Exception as e:
            print(f"[ERROR] 批量建立提醒失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
