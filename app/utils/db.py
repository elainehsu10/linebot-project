...
class DB:
    ...
    def get_prescription_for_liff(mm_id):
        """为LIFF页面获取处方信息"""
        try:
            print(f"[DEBUG] 獲取藥歷資料用於LIFF，mm_id: {mm_id}")
            
            prescription = DB.get_prescription_by_mm_id(mm_id)
            if not prescription:
                print(f"[ERROR] 找不到藥歷記錄，mm_id: {mm_id}")
                return None
            
            # 準備 LIFF 頁面需要的資料格式
            result = {
                'mm_id': prescription['mm_id'],
            ...
            print(f"[DEBUG] 藥物數量: {len(medications)}")
            ...
            for med in medications:
                ...
                result['medications'].append(med_data)
            
            # 根據頻率代碼生成預設提醒時間
            frequency_presets = {
                'QD': ['08:00'],  # 一日一次
                'BID': ['08:00', '20:00'],  # 一日兩次
                'TID': ['08:00', '14:00', '20:00'],  # 一日三次
                'QID': ['08:00', '12:00', '16:00', '20:00'],  # 一日四次
            }
            ...
            for med in medications:
                ...
                if freq_code and freq_code not in used_frequencies:
                    if freq_code in frequency_presets:
                        result['presets'].append({
                            'frequency': freq_code,
                            'times': frequency_presets[freq_code]
                        })
                        used_frequencies.add(freq_code)
            
            print(f"[DEBUG] LIFF 資料準備完成，藥物: {len(result['medications'])}, 預設: {len(result['presets'])}")
            return result
            
        except Exception as e:
            print(f"[ERROR] 獲取LIFF藥歷資料失敗: {e}")
            import traceback
            traceback.print_exc()
            return None
        ...
        try:
            with db.cursor() as cursor:
                ...
                for i, reminder in enumerate(reminders_data):
                    try:
                        print(f"[DEBUG] 處理提醒 {i+1}/{len(reminders_data)}: {reminder.get('drug_name', 'Unknown')}")
                        
                        # 驗證必要欄位
                        required_fields = ['recorder_id', 'member', 'drug_name']
                        for field in required_fields:
                            if not reminder.get(field):
                                print(f"[ERROR] 提醒 {i+1} 缺少必要欄位: {field}")
                                continue
                        
                        # 驗證成員是否存在
                        cursor.execute("SELECT COUNT(*) as count FROM members WHERE recorder_id = %s AND member = %s", 
                                     (reminder['recorder_id'], reminder['member']))
                        member_exists = cursor.fetchone()['count'] > 0
                        
                        if not member_exists:
                            print(f"[WARNING] 成員不存在，自動建立: {reminder['member']}")
                            cursor.execute("INSERT INTO members (recorder_id, member) VALUES (%s, %s)", 
                                         (reminder['recorder_id'], reminder['member']))
                        
                        # 建立提醒
                        result = DB.create_reminder(reminder)
                        if result:
                            success_count += 1
                            print(f"[SUCCESS] 提醒 {i+1} 建立成功，ID: {result}")
                        else:
                            print(f"[ERROR] 提醒 {i+1} 建立失敗")
                            
                    except Exception as reminder_error:
                        print(f"[ERROR] 處理提醒 {i+1} 時發生錯誤: {reminder_error}")
                        continue
                
                db.commit()
                print(f"[SUCCESS] 批量建立提醒完成，成功: {success_count}/{len(reminders_data)}")
                return success_count > 0
                
        except Exception as e:
            print(f"[ERROR] 批量创建提醒失败: {e}")
            import traceback
            traceback.print_exc()
            db.rollback()
            return False
